...
def evaluate(style_model ,content_image,content_size,style_image,style_size,cuda,output_image='./output.jpg'):
    content_image = utils.tensor_load_rgbimage(content_image, size=content_size, keep_asp=True)
    content_image = content_image.unsqueeze(0)
    style = utils.tensor_load_rgbimage(style_image, size=style_size,style=True)
    ...
    #utils.tensor_save_bgrimage(output.data[0], output_image, cuda)
   # print('i am 333',output.data[0])
   # print('i am 333',output.data[0].size())

    #(b, g, r) = torch.chunk(output.data[0], 3)
    #tensor = torch.cat((r, g, b))

    #array1 = output.data[0].permute(1,2,0).cpu().numpy()
    output = output.cpu().clamp(0, 255).data[0].numpy()
